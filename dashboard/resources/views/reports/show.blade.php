...
<div class="card p-3 mb-3">
    <h5>Exam Session</h5><p>{{ $analysisJob->session->name ?? "—" }} (Room: {{ $analysisJob->session->room->name ?? "—" }})</p>
    ...
    <h5>Analysis Job</h5><p>ID: {{ $analysisJob->id }}<br>Status: <span class="badge bg-info">{{ $analysisJob->status }}</span> Progress: {{ $analysisJob->progress_percent }}%<br>Remote: {{ $analysisJob->remote_job_id ?? "—" }} ({{ $analysisJob->remote_status ?? "—" }})<br>Started: {{ $analysisJob->started_at ?? "—" }} Completed: {{ $analysisJob->completed_at ?? "—" }}<br>Failure: {{ $analysisJob->failure_reason ?? "—" }}</p>
    <h5>Model Version</h5><p>{{ $analysisJob->modelVersion->name ?? "—" }} {{ $analysisJob->modelVersion->version ?? "" }} ({{ $analysisJob->modelVersion->license ?? "" }}) Checksum: {{ $analysisJob->modelVersion->checksum_sha256 ?? "" }}</p>
