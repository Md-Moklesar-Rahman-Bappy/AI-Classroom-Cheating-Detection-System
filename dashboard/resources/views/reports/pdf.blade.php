...
<p>Session: {{ $analysisJob->session->name ?? "Not assigned" }}</p><p>Job: {{ $analysisJob->id }} Status: {{ $analysisJob->status }}</p><p>Model: {{ $analysisJob->modelVersion->name ?? "Not assigned" }} {{ $analysisJob->modelVersion->version ?? "" }}</p>
