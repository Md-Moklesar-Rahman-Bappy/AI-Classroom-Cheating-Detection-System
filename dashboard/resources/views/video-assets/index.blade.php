...
@else<div class="table-responsive"><table class="table"><thead><tr><th>SL</th><th>Original</th><th>Stored</th><th>Mime</th><th>Status</th><th>Linked Jobs</th><th>Actions</th></tr></thead><tbody>@foreach($assets as $i => $a)<tr><td>{{ $i + 1 }}</td><td>{{ $a->original_filename }}</td><td class="text-muted small">{{ Str::limit($a->stored_filename,20) }}</td><td>{{ $a->mime_type }}</td><td><span class="badge @if($a->validation_status=="valid") bg-success @elseif($a->validation_status=="invalid") bg-danger @else bg-warning text-dark @endif">{{ $a->validation_status }}</span></td><td>{{ $a->linkedJobCount }}</td><td>
