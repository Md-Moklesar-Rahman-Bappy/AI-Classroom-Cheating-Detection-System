...
<div class="d-flex justify-content-between align-items-center"><span class="badge bg-light text-dark border" style="font-size:10px">{{ $metrics->firstItem()+$i }}</span><span class="text-truncate" style="max-width:60%">{{ $m->analysisJob?->videoAsset?->original_filename ?? $m->analysisJob?->session?->name ?? "Unknown job" }}</span><span class="text-muted" style="font-size:11px">{{ $m->processing_fps ?? "—" }} FPS</span></div>
