...
test('multipart transfer succeeds', function () {
    ...
    Http::fake(['*' => Http::response(['job_id' => (string) Str::uuid(), 'status' => 'queued', 'progress_percent' => 0], 201)]);
...
test('FastAPI creates its own safe temporary input', function () {
    Http::fake(['*' => Http::response(['job_id' => (string) Str::uuid(), 'status' => 'pending'], 201)]);
    $client = app(AiServiceClient::class);
    $tmp = tempnam(sys_get_temp_dir(), 'test');
    file_put_contents($tmp, 'fake video');
    $result = $client->createRecordedJob($tmp, 'test.mp4', (string) Str::uuid(), 'video/mp4', 1024, hash('sha256', 'fake video'), 'yolo11n.pt', [], 1);
...
test('Laravel saves remote_job_id', function () {
    ...
    $session = ExamSession::create(['name' => 'Test', 'status' => 'pending', 'created_by' => $admin->id]);
    $model = ModelVersion::active()->first();
    $job = AnalysisJob::create(['exam_session_id' => $session->id, 'source_type' => 'recorded_video', 'model_version_id' => $model->id, 'status' => 'queued', 'config' => [], 'created_by' => $admin->id, 'correlation_id' => (string) Str::uuid(), 'remote_job_id' => (string) Str::uuid()]);
...
test('status advances beyond queued', function () {
    ...
    $session = ExamSession::create(['name' => 'Test', 'status' => 'pending', 'created_by' => $admin->id]);
    $model = ModelVersion::active()->first();
    $job = AnalysisJob::create(['exam_session_id' => $session->id, 'source_type' => 'recorded_video', 'model_version_id' => $model->id, 'status' => 'queued', 'config' => [], 'created_by' => $admin->id, 'correlation_id' => (string) Str::uuid(), 'remote_job_id' => (string) Str::uuid(), 'remote_status' => 'processing', 'progress_percent' => 50]);
...
test('duplicate request is idempotent or safely rejected', function () {
    ...
    Storage::disk('local')->put('video_assets/'.$stored, 'fake');
    $asset = VideoAsset::create(['exam_session_id' => $session->id, 'original_filename' => 'video.mp4', 'stored_filename' => $stored, 'mime_type' => 'video/mp4', 'size_bytes' => 100, 'validation_status' => 'valid', 'uploaded_by' => $admin->id]);
    Http::fake(['*' => Http::response(['job_id' => (string) Str::uuid(), 'status' => 'queued'], 201)]);
    $this->actingAs($admin)->post(route('analysis-jobs.store'), ['exam_session_id' => $session->id, 'source_type' => 'recorded_video', 'model_version_id' => $model->id, 'video_asset_id' => $asset->id]);
...
test('existing event/evidence synchronization remains functional', function () {
    ...
    $session = ExamSession::create(['name' => 'Test', 'status' => 'pending', 'created_by' => $admin->id]);
    $model = ModelVersion::active()->first();
    $job = AnalysisJob::create(['exam_session_id' => $session->id, 'source_type' => 'test_source', 'model_version_id' => $model->id, 'status' => 'completed', 'config' => [], 'created_by' => $admin->id, 'correlation_id' => (string) Str::uuid()]);
...
test('health check retry callback does not call invalid PendingRequest::method', function () {
    ...
    $client = new AiServiceClient('http://127.0.0.1:8001', 'test', 1);
    try {
        $client->healthCheck((string) Str::uuid());
    } catch (AiServiceException $e) {
