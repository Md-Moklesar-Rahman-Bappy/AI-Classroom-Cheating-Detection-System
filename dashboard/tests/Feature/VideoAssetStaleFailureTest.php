...
test('valid asset does not trigger stale video missing', function () {
    ...
    $job = AnalysisJob::create(['exam_session_id' => $session->id, 'source_type' => 'recorded_video', 'video_asset_id' => $asset->id, 'model_version_id' => $model->id, 'status' => 'pending', 'config' => [], 'created_by' => $admin->id, 'correlation_id' => (string) \Illuminate\Support\Str::uuid()]);
...
test('missing file uses accurate failure not stale', function () {
    ...
    $job = AnalysisJob::create(['exam_session_id' => $session->id, 'source_type' => 'recorded_video', 'video_asset_id' => null, 'model_version_id' => $model->id, 'status' => 'pending', 'config' => [], 'created_by' => $admin->id, 'correlation_id' => (string) \Illuminate\Support\Str::uuid()]);
