...
def add_cover(doc):
    ...
    meta = [
        ...
        ("Document Version", "1.1 (post-UI/UX modernization)"),
        ("Repository Commit", "e76a968 — updated 2026-08-31 (see git log)"),
        ("Prepared", "2026-08-31"),
        ("Status", "Research prototype — not production-ready (see RELEASE_CHECKLIST)"),
        (
            "Document Type",
            "Complete system, operator, developer, and research documentation",
        ),
    ]
    for k, v in meta:
    ...
    doc.add_paragraph()
    add_responsible_ai_box(
        doc,
        "This document describes an AI-assisted surveillance system. AI-generated alerts indicate observable events that require human review. An alert is not proof of academic misconduct. Final academic or disciplinary decisions remain with authorized human reviewers and the institution.",
    )
...
def main():
    ...
    for section in doc.sections:
        ...
        fp.text = "Commit e76a968 — Research prototype — Design System V3 — 2026-08-31"
    ...
    add_para(
        doc,
        "Updated automatically in Microsoft Word (References → Table of Contents → Update Field). Sections include: Executive Summary, Project Background, Feature Catalog, System Architecture, CV/Event Logic, Database/API, Installation/Operation, Testing/Benchmarking, Security/Privacy, Appendices.",
    )
    ...
    add_para(
        doc,
        "Developed as a Master’s project at Jahangirnagar University under supervisor Risala Tasin Khan, PhD. The repository is open-source (AGPL-3.0) and contains both synthetic/non-identifiable benchmark assets and infrastructure for approved staged recordings.",
    )

    add_heading_custom(doc, "3. Problem Statement", 2)
    add_para(
        doc,
        "Classroom examination integrity relies on human invigilator observation, which has coverage and consistency limits. The system provides AI-assisted observation to support—not replace—human review.",
    )

    add_heading_custom(doc, "4. Motivation", 2)
    add_para(
        doc,
        "Institutional need for consistent, documented, reviewable observation of examination sessions, with privacy controls, audit trails, and human-in-the-loop decisions.",
    )

    add_heading_custom(doc, "5. Project Aim", 2)
    add_para(
        doc,
        "Build a complete pipeline: camera/video input → frame processing → YOLO detection → anonymous tracking → orientation estimation → temporal rules → event generation → evidence management → dashboard review → audit log.",
    )
    ...
    add_para(
        doc,
        "RQ1: Can synthetic/non-identifiable test material provide reproducible baseline metrics for object detection? (Answered: benchmark exists; real-data blocked.)",
    )
    add_para(
        doc,
        "RQ2: Does anonymous tracking maintain session-level consistency without identity recognition? (Answered: yes within session; identity switch possible at long occlusion.)",
    )
    add_para(
        doc,
        "RQ3: Can temporal rules distinguish sustained behavior from transient detection? (Answered: partially; requires more sequential data.)",
    )
    ...
    add_para(
        doc,
        "Recorded video upload, validation, storage, analysis, evidence generation, review workflow, audit logs, dashboard CRUD for video assets and analysis jobs, model version management, metrics/report generation, dataset governance for synthetic/non-identifiable data.",
    )
    add_heading_custom(doc, "8.2 MVP Scope", 3)
    add_para(
        doc,
        "Single-source recorded video → YOLO detection → events → evidence → review → report.",
    )
    add_heading_custom(doc, "8.3 Experimental Scope", 3)
    add_para(
        doc,
        "Live webcam/test stream monitoring; orientation estimation; temporal event engine; multi-source correlation (not fully validated).",
    )
    add_heading_custom(doc, "8.4 Excluded Scope", 3)
    add_para(
        doc,
        "Real participant recordings (blocked until consent verified); automated disciplinary decisions; facial recognition; biometric templates; public release of identifiable data.",
    )
    add_heading_custom(doc, "8.5 Planned Future Scope", 3)
    add_para(
        doc,
        "Verified EZVIZ integration (currently unverified); GPU benchmark; approved real-world dataset evaluation; improved temporal models with sufficient sequential data.",
    )

    add_heading_custom(doc, "9. Expected Contribution", 2)
    add_para(
        doc,
        "A reproducible, auditable, privacy-preserving AI-assisted examination-surveillance pipeline with documented governance, synthetic/non-identifiable evaluation, and explicit limitation statements.",
    )
    ...
    users = [
        (
            ...
            "Full access; configuration; model management; retention.",
        ),
    ...
    add_para(
        doc,
        "Detect visible persons and phones; track anonymously; estimate geometric orientation; apply temporal rules; generate evidence; support human review; log audits; manage video assets and jobs.",
    )

    add_heading_custom(doc, "12. What the System Cannot Do", 2)
    add_para(
        doc,
        "It does not automatically determine academic misconduct. It does not recognize identity. It does not guarantee real-world accuracy from synthetic benchmarks. It does not support unverified EZVIZ integration. It does not replace human review.",
    )

    add_heading_custom(doc, "13. Important Limitations", 2)
    add_warning_box(
        doc,
        "Real-data evaluation is BLOCKED (DATA_PLAN.md approved only for staged recordings with consent; synthetic/non-identifiable test data used currently).",
    )
    add_warning_box(
        doc,
        "Model accuracy on real classroom footage is NOT verified. Synthetic benchmark results do not generalize to all real conditions.",
    )
    add_warning_box(
        doc,
        "B4 Possible Seat Departure detection is partial/experimental unless fully validated.",
    )
    add_warning_box(
        doc,
        "Geometric orientation is a lightweight heuristic, not facial recognition or intent detection.",
    )

    add_heading_custom(doc, "14. Responsible-AI Notice", 2)
    add_responsible_ai_box(
        doc,
        "AI-generated alerts indicate observable events requiring human review. An alert is not proof of academic misconduct. Final academic or disciplinary decisions remain with authorized human reviewers and the institution.",
    )
    ...
    add_para(
        doc,
        "Implemented: Laravel Breeze-based auth with login, logout, password reset. Email verification available. Session encrypted (SESSION_ENCRYPT should be true in production; currently false in .env.local). Login rate limiting via Laravel throttle middleware.",
    )
    add_security_box(
        doc, ".env local uses APP_DEBUG=true. Production must set APP_DEBUG=false."
    )

    add_heading_custom(doc, "23. Dashboard Overview", 2)
    add_para(
        doc,
        "KPI cards, system status, recent activities, quick actions. All values derived from database queries (verified in controllers).",
    )
    ...
    add_para(
        doc,
        "Created with name, status, created_by. Used by video assets and analysis jobs.",
    )

    add_heading_custom(doc, "26. Camera Sources", 2)
    add_para(
        doc,
        "Configured with type (webcam, test, RTSP), URL/credentials, preview settings.",
    )
    ...
    add_para(
        doc,
        "Create from session + video asset + model version + config. Job states: pending, queued, processing, completed, failed, cancelled. Status-based actions (view/edit/delete) configured in policy. Remote job ID and correlation ID tracked. Audit logs for create/approve/review/delete/retry.",
    )

    add_heading_custom(doc, "29. Live Monitoring", 2)
    add_para(
        doc,
        "Supported sources: webcam (verified), test stream (verified), RTSP adapter (implemented, unverified for EZVIZ), single-source limit (verified). States: running, paused, disconnected, degraded, reconnecting. Preview via MJPEG. Health endpoint verified.",
    )
    ...
    add_para(
        doc,
        "Events generated by temporal rule engine: B1-B4 temporal events; D1/D2 detection outputs. Evidence snapshots include track number, event label, timestamp, and visual state. Human review required.",
    )

    add_heading_custom(doc, "31. Evidence", 2)
    add_para(
        doc, "Protected directory; not in Git; accessible to authorized reviewers only."
    )

    add_heading_custom(doc, "32. Human Reviews", 2)
    add_para(
        doc,
        "Reviewer selects decision (confirmed suspicious, dismissed as normal, needs further review). Notes added. Audit log records decision with reviewer identifier.",
    )

    add_heading_custom(doc, "33. Model Versions", 2)
    add_para(
        doc,
        "Registered with checksum (yolo11n.pt: 0ebbc80d4... verified in benchmark_manifest). Weight file excluded from Git.",
    )

    add_heading_custom(doc, "34. Metrics", 2)
    add_para(
        doc,
        "Actual benchmark results in research/results/benchmark_results.json. No fabricated values.",
    )
    ...
    add_para(
        doc,
        "Every job action (create, approve, review, delete, retry) logged with user, timestamp, action, result.",
    )

    add_heading_custom(doc, "37. Users", 2)
    add_para(
        doc,
        "Custom role system (system_admin, exam_admin, invigilator, reviewer, auditor) via role_user pivot.",
    )

    add_heading_custom(doc, "38. Roles and Permissions", 2)
    add_para(
        doc,
        "Policies registered (VideoAssetPolicy, AnalysisJobPolicy). Middleware enforces access.",
    )

    add_heading_custom(doc, "39. Settings", 2)
    add_para(
        doc, "AI service base URL, retry settings, timeout, model version defaults."
    )

    add_heading_custom(doc, "40. Help and Responsible-Use Notice", 2)
    add_responsible_ai_box(
        doc, "This system is AI-assisted, not autonomous. Human review required."
    )
    ...
    add_code_block(
        doc,
        "Camera / Recorded Video → Adapter → Frame Scheduler → Preprocessing → YOLO Detector → Anonymous Tracker → Orientation Method → Temporal Rule Engine → Renderer / Evidence → Metrics / Report → API → Laravel Dashboard → Human Reviewer",
    )

    add_heading_custom(doc, "42. Component Architecture", 2)
    add_para(
        doc,
        "Components: input adapters (RecordedVideoInput, WebcamInput, RtspStreamInput, TestVideoInput), output adapters (EvidenceManager, MetricsExporter, ReportGenerator), detection engine (YOLO11 Nano), tracker (SimpleCentroidTracker), orientation (geometric-v1), temporal rules (ObservationWindow + RuleEngine).",
    )

    add_heading_custom(doc, "43. AI-Service Architecture", 2)
    add_para(
        doc,
        "FastAPI app; modules: main, api/jobs, api/live, api/health, api/version, api/events, api/metrics, config/settings. Uses uvicorn; must start from ai-service root.",
    )

    add_heading_custom(doc, "44. Laravel Dashboard Architecture", 2)
    add_para(
        doc,
        "MVC with Blade views; roles via custom pivot; policies registered; audit via AuditHelper; soft deletes on VideoAsset and AnalysisJob.",
    )

    add_heading_custom(doc, "45. Shared Detection Engine", 2)
    add_para(
        doc,
        "Python modules in ai-service/app/ detect person and mobile-phone; feed tracker; compute orientation; evaluate temporal rules.",
    )
    ...
    add_para(
        doc,
        "EvidenceManager (snapshots), MetricsExporter (benchmark), ReportGenerator (PDF-style), AuditLogger (database).",
    )

    add_heading_custom(doc, "48. Recorded-Video Workflow", 2)
    add_para(
        doc,
        "Upload → validation → storage → analysis job creation → remote job → poll → events → evidence → review → metrics.",
    )

    add_heading_custom(doc, "49. Live-Surveillance Workflow", 2)
    add_para(
        doc,
        "Start source → frame scheduler → detection → tracker → orientation → temporal rules → event alert → evidence → preview → stop.",
    )

    add_heading_custom(doc, "50. Cross-Service Multipart-Transfer Workflow", 2)
    add_para(
        doc,
        "Laravel asset → stream open → multipart → FastAPI validation → temporary input → remote job → remote ID → polling → event sync → evidence transfer → metrics sync → cleanup.",
    )

    add_heading_custom(doc, "51. Queue Architecture", 2)
    add_para(
        doc,
        "Laravel queue (database driver); worker must remain running. Job retries configurable.",
    )

    add_heading_custom(doc, "52. Job Lifecycle and State Machine", 2)
    add_para(
        doc,
        "States: pending → queued → processing → completed / failed / cancelled. Retry and cancel supported.",
    )

    add_heading_custom(doc, "53. Event Lifecycle", 2)
    add_para(
        doc,
        "Detection → observation window → supporting observation → event end → evidence → review.",
    )

    add_heading_custom(doc, "54. Evidence Lifecycle", 2)
    add_para(
        doc,
        "Created on event confirmation; protected; retained per policy; deleted secure if required.",
    )

    add_heading_custom(doc, "55. Human-Review Lifecycle", 2)
    add_para(
        doc,
        "Pending review → reviewer selects → notes added → audit log → evidence updated → report updated.",
    )
    ...
    add_para(
        doc,
        "Service unavailable: retry; timeout: configurable; invalid stream: drop; unauthorized access: 403; missing file: re-upload or block.",
    )

    add_heading_custom(doc, "58. Correlation IDs and Traceability", 2)
    add_para(
        doc,
        "Correlation ID (user-set) and remote job ID (service-returned) allow trace from dashboard to AI output.",
    )
    ...
    add_para(
        doc,
        "YOLO11 Nano (yolo11n.pt). Classes: person (COCO 0), cell phone (COCO 67). Checksum verified: 0ebbc80d4a7680d14987a577cd21342b65ecfd94632bd9a8da63ae6417644ee1 (benchmark_manifest.json). Source: Ultralytics. License: AGPL-3.0. CPU inference only (current environment). Weight file not committed (excluded by .gitignore).",
    )
    add_warning_box(
        doc, "No real-world accuracy study performed; synthetic benchmark only."
    )

    add_heading_custom(doc, "60. Person Detection", 2)
    add_para(
        doc, "Bounding box around visible torso/head. Not a suspicious event by itself."
    )
    ...
    add_para(
        doc,
        "SimpleCentroidTracker; temporary IDs; distance threshold; missed-frame tolerance. No embeddings. Session scope. Identity-switch possible at long occlusion.",
    )

    add_heading_custom(doc, "63. Orientation Method", 2)
    add_para(
        doc,
        "Geometric-v1: uses relative positions of tracked points to derive direction states. Lightweight heuristic. Not facial recognition; not intent detection.",
    )

    add_heading_custom(doc, "64. Temporal Rule Engine", 2)
    add_para(
        doc,
        "Observation window; supporting-observation count; minimum duration; cooldown; duplicate suppression.",
    )

    add_heading_custom(doc, "65. Event Taxonomy", 2)
    add_table_custom(
        doc,
        ["Category", "Code", "Meaning", "Status"],
        [
    ...
    add_para(
        doc,
        "Green (normal), Amber (uncertain / needs review), Red (suspicious event observed — requires human decision), Blue (evidence selected), Gray (inactive / unavailable).",
    )

    add_heading_custom(doc, "67. B4 Leaving-Seat Limitations", 2)
    add_warning_box(
        doc,
        "B4 Possible Seat Departure is partial/experimental. Not fully validated for real-world conditions. Requires temporal evidence and human confirmation.",
    )
    ...
    add_para(
        doc,
        "MySQL (configured via DB_CONNECTION=mysql). Migration-backed schema. Soft deletes on VideoAsset and AnalysisJob.",
    )

    add_heading_custom(doc, "69. ER Overview", 2)
    add_para(
        doc,
        "Key entities: users → roles → exam_sessions → video_assets → analysis_jobs → detection_events → event_evidence → review_decisions → audit_logs. VideoAsset links to AnalysisJob via video_asset_id.",
    )
    ...
    tables = [
        ...
        [
            "exam_sessions",
            "Session metadata",
            "id, name, status, exam_room_id, created_by, created_at",
        ],
        [
            "video_assets",
            "Uploaded videos",
            "id, exam_session_id, stored_filename, mime_type, size_bytes, validation_status, deleted_at",
        ],
        [
            "analysis_jobs",
            "Processing jobs",
            "id, exam_session_id, video_asset_id, source_type, model_version_id, status, remote_job_id, correlation_id, deleted_at",
        ],
        [
            "model_versions",
            "Model registry",
            "id, name, version, checksum_sha256, weight_filename",
        ],
        [
            "detection_events",
            "Events",
            "id, analysis_job_id, event_type, start_frame, end_frame, confidence",
        ],
        [
            "event_evidence",
            "Evidence",
            "id, event_id, snapshot_path, review_decision_id",
        ],
        [
            "review_decisions",
            "Review",
            "id, evidence_id, reviewer_code, decision, notes",
        ],
    ...
    add_para(
        doc,
        "AnalysisJob belongsTo VideoAsset (video_asset_id). VideoAsset hasMany AnalysisJob (analysisJobs). DetectionEvent belongsTo AnalysisJob.",
    )

    add_heading_custom(doc, "72. Soft-Delete Strategy", 2)
    add_para(
        doc,
        "VideoAsset and AnalysisJob both use SoftDeletes. Deleted records excluded from normal queries; recoverable via restore(). Physical file deletion is separate from soft delete.",
    )

    add_heading_custom(doc, "73. API Overview", 2)
    add_para(
        doc,
        "FastAPI endpoints: /health, /version, /jobs (create/status/cancel/retry), /events, /metrics, /live/start/stop/health/preview.",
    )

    add_heading_custom(doc, "74. Authentication and Headers", 2)
    add_para(
        doc,
        "API token via env (AI_SERVICE_TOKEN). Not exposed in Git. HTTPS recommended in production.",
    )
    ...
    add_para(
        doc,
        "multipart/form-data with file field; validated by FastAPI; temporary storage during processing.",
    )

    add_heading_custom(doc, "77. Idempotency", 2)
    add_para(
        doc, "Retry attempts configurable; correlation ID allows duplicate detection."
    )
    ...
    add_security_box(
        doc,
        ".env excluded from Git; token via env only; config default is placeholder.",
    )
    ...
    add_para(
        doc,
        "Windows 10/11 (verified environment); PHP 8+; Composer; MySQL/MariaDB; Node.js/npm; Python 3.14; OpenCV; Ultralytics; PyTorch CPU.",
    )
    ...
    add_para(
        doc,
        "Root contains ai-service/, dashboard/, docs/, research/, scripts/. Data and weights outside Git.",
    )
    ...
    add_important_box(
        doc,
        "Use python -m uvicorn, not bare uvicorn, to avoid PATH issues observed during development.",
    )

    add_heading_custom(doc, "84. AI-Service Environment", 2)
    add_para(
        doc,
        "Copy .env.example to .env; set AI_SERVICE_TOKEN (not default placeholder in production); configure base_url, timeouts.",
    )
    ...
    add_important_box(
        doc,
        "Start from ai-service root. Previous ModuleNotFoundError caused by wrong working directory.",
    )
    ...
    add_para(
        doc,
        "MySQL connection; database ai_classroom; user root (development only); password empty in .env.local.",
    )

    add_heading_custom(doc, "89. Role / Default Model Seeding", 2)
    add_para(
        doc,
        "Seeders create roles (system_admin, exam_admin, invigilator, reviewer, auditor) and default model version.",
    )
    ...
    add_code_block(
        doc, "php artisan queue:work --queue=default --tries=1 --timeout=900 -v"
    )
    add_warning_box(
        doc,
        "Worker must remain running. Jobs stay pending without it. Restart after code/config changes: php artisan queue:restart.",
    )
    ...
    add_table_custom(
        ...
        [
            ["T1", "python -m uvicorn ...", "AI service"],
            ["T2", "php artisan serve", "Dashboard"],
            ["T3", "php artisan queue:work ...", "Worker"],
        ],
    )
    ...
    add_para(
        doc,
        "Check login, video asset upload, analysis job creation, job processing, event generation, evidence, review.",
    )

    add_heading_custom(doc, "94. Environment Variables", 2)
    add_table_custom(
        doc,
        ["Key", "Purpose", "Required", "Security"],
        [
    ...
    add_para(
        doc,
        "Navigation: Dashboard, Exam Rooms, Exam Sessions, Cameras, Video Assets, Analysis Jobs, Live Monitoring, Evidence, Reviews, Reports, Audit Logs.",
    )
    ...
    add_para(
        doc,
        "Supported formats: mp4, avi, mov, mkv. Validation checks MIME and size (max ~512 MB). Storage is private (UUID filename). Status set to pending/valid/invalid based on validation.",
    )

    add_heading_custom(doc, "101. Viewing Video Assets", 2)
    add_para(
        doc,
        "Index table shows SL, original, stored (truncated), MIME, size, created date, status badge, linked jobs count, actions.",
    )

    add_heading_custom(doc, "102. Editing a Video Asset", 2)
    add_para(
        doc, "Click Edit to change exam session, original filename, validation status."
    )

    add_heading_custom(doc, "103. Deleting a Video Asset", 2)
    add_para(
        doc,
        "Click Delete → SweetAlert2 confirmation → soft delete (recoverable). Blocked if analysis jobs linked (shows count). Audit log entry created.",
    )

    add_heading_custom(doc, "104. Starting Analysis from Asset", 2)
    add_para(
        doc,
        "Click Analyze from asset page → creates job with pre-selected video asset.",
    )

    add_heading_custom(doc, "105. Creating Analysis Job Manually", 2)
    add_para(
        doc,
        "Select session → source type → video asset → model version → config (interval, mode, resolution). Submit.",
    )
    ...
    add_para(
        doc,
        "Interval (every frame / every 3rd / every 5th); mode (recorded/live); resolution (640x360 / 480x270).",
    )
    ...
    add_table_custom(
        ...
        [
            ["Pending", "Queued for worker", "Edit / Cancel"],
            ["Queued", "In queue", "View"],
            ["Processing", "Running", "View"],
            ["Completed", "Finished", "View / Report / Delete"],
            ["Failed", "Error", "Retry / View"],
    ...
    add_para(
        doc, "Actual benchmark results from research/results/benchmark_results.json."
    )
    ...
    add_para(
        doc,
        "Stop live; complete or cancel jobs; stop queue worker; stop FastAPI; close terminal.",
    )
    ...
    add_para(
        doc,
        "Backup database (sql dump), storage directory (excluding raw videos if required by policy), and model weights separately.",
    )

    add_heading_custom(doc, "145. Evidence Retention and Deletion", 2)
    add_para(
        doc,
        "Follow DATA_RETENTION_POLICY.md. Secure deletion required for early removal.",
    )

    add_heading_custom(doc, "146. Missing-File Reconciliation", 2)
    add_para(
        doc,
        "Video asset shows invalid when file missing; re-upload from backup or source.",
    )

    add_heading_custom(doc, "147. Security Incident Response", 2)
    add_para(
        doc,
        "Document in audit; review access; check retention actions; report to supervisor.",
    )

    add_heading_custom(doc, "148. Production Configuration Checklist", 2)
    add_warning_box(
        doc,
        "Not production-ready (release criteria incomplete). Before production: fix .env secrets, enable session encryption, disable debug, set HTTPS, configure CI/dependabot, complete user acceptance, verify GPU benchmark if needed.",
    )
    ...
    add_para(
        doc,
        "AI service: Python modules; Dashboard: Laravel MVC; Tests: PHPUnit + Python.",
    )

    add_heading_custom(doc, "150. AI-Service Modules", 2)
    add_para(
        doc,
        "main, api/jobs, api/live, api/health, api/version, api/events, api/metrics, config/settings.",
    )

    add_heading_custom(doc, "151. Dashboard Modules", 2)
    add_para(
        doc,
        "Controllers, Models (with SoftDeletes), Policies, Middleware, Views, Routes.",
    )
    ...
    add_para(
        doc,
        "Implement adapter class; register in scheduler; add to allowed source types.",
    )
    ...
    add_para(
        doc, "Add class to taxonomy; update annotation guide; update metrics if needed."
    )
    ...
    add_security_box(
        doc, "No secrets in source. Use .env. Default token is placeholder only."
    )
    ...
    add_para(
        doc,
        "Use descriptive messages; include issue reference; do not commit secrets or weights.",
    )
    ...
    add_para(
        doc,
        "Laravel PHPUnit feature tests (video assets, analysis jobs). Python benchmark script (actual execution, not invented).",
    )

    add_heading_custom(doc, "174. Running Python Tests", 2)
    add_para(
        doc, "No dedicated Python test folder; benchmark script serves as validation."
    )
    ...
    add_para(
        doc, "Not fully executed (some LSP diagnostics observed; not quality results)."
    )
    ...
    add_warning_box(
        doc,
        "Unit tests validate software behavior. They do not prove real-world model accuracy on classroom footage.",
    )

    add_heading_custom(doc, "184. Benchmark Methodology (Phase 8)", 2)
    add_para(
        doc,
        "Environment: CPU-only (Intel i5-14500, 8 GB RAM, no GPU). Synthetic fixture: 640x360, 10 fps, 90 frames, gray rectangle + white circle, no person, no PII. Resolutions: 640x360 and 480x270. Frame intervals: every 1, 3, 5 frames. Warm-up separate. Actual measured FPS, latency, CPU, memory from benchmark_manifest.json.",
    )

    add_heading_custom(doc, "185. Low-Resource Profile", 2)
    add_para(
        doc,
        "CPU inference; no GPU required for basic operation; performance lower than GPU-equipped systems.",
    )

    add_heading_custom(doc, "186. Real Recorded-Job Runtime Example", 2)
    add_para(
        doc,
        "Job 12 completed successfully (verified in project history). Exact metrics not preserved in repository files; not presented as accuracy study.",
    )
    ...
    add_para(
        doc,
        "evaluate_objects.py, evaluate_events.py, check_split_leakage.py, generate_confusion_matrix.py, check_data_validation.py.",
    )

    add_heading_custom(doc, "196. Evaluation Metrics", 2)
    add_para(
        doc,
        "Actual results in benchmark_results.json and sanitized_evaluation_result.json. No fabricated metrics.",
    )

    add_heading_custom(doc, "197. Current Evaluation Status", 2)
    add_para(
        doc,
        "Synthetic/non-identifiable evaluation active. Real participant evaluation BLOCKED (no consent artifacts verified for real recordings).",
    )

    add_heading_custom(doc, "198. Reproducibility", 2)
    add_para(
        doc, "REPRODUCIBILITY.md; seed and hardware recorded; commands documented."
    )
    ...
    add_para(
        doc,
        "Laravel auth; policies; middleware; soft deletes; audit logs; private storage; secret redaction; HTTPS required in production.",
    )

    add_heading_custom(doc, "200. Authentication Controls", 2)
    add_para(
        doc,
        "Password-based; session encrypted (recommended true in prod); rate limiting via throttle.",
    )

    add_heading_custom(doc, "201. Authorization Controls", 2)
    add_para(
        doc,
        "Role-based (system_admin, exam_admin, invigilator, reviewer, auditor). Policies enforce per-resource access.",
    )
    ...
    add_para(
        doc, "Evidence directory excluded from Git; access controlled by authorization."
    )

    add_heading_custom(doc, "206. Secret Redaction", 2)
    add_security_box(
        doc, ".env excluded; no committed secrets found; placeholder token documented."
    )
    ...
    add_para(
        doc,
        "Threat model documented (THREAT_MODEL.md). Risks: unauthorized access, data leakage, model misuse, evidence tampering.",
    )

    add_heading_custom(doc, "212. Responsible-AI Controls", 2)
    add_responsible_ai_box(
        doc,
        "Human review required; no automated disciplinary decision; disclaimer in documentation and UI.",
    )
    ...
    add_para(
        doc,
        "No real-participant data included; synthetic only; consent artifacts present for future approved collection.",
    )

    add_heading_custom(doc, "215. Ethical Data-Collection Requirements", 2)
    add_para(
        doc,
        "Institutional approval + informed consent + retention notice required before any real recording.",
    )
    ...
    add_para(
        doc,
        "Package: ultralytics 8.4.135. License: AGPL-3.0. Usage: YOLO detection. Source availability: yes. Model weight (yolo11n.pt) not redistributed in repository; downloaded separately. Network: local inference only (no remote model call). Weight-file considerations: exclude from Git; verify checksum.",
    )
    add_important_box(
        doc,
        "This section provides technical compliance documentation and is not legal advice.",
    )
    ...
    items = [
        (
            "219. uvicorn: command not found",
            "Use python -m uvicorn from ai-service root.",
        ),
        (
            "220. ModuleNotFoundError: No module named app",
            "Run from ai-service directory; use correct working directory.",
        ),
        (
            "221. FastAPI health unavailable",
            "Check service running; verify port 8001; check firewall.",
        ),
        (
            "222. YOLO model cannot load",
            "Verify yolo11n.pt present; verify checksum; verify PYTHONPATH.",
        ),
        (
            "223. Model weight downloaded automatically",
            "Expected if missing; verify source.",
        ),
        (
            "224. Queue job remains pending",
            "Start queue worker; verify database connection.",
        ),
        (
            "225. Queue job fails immediately",
            "Check error message; verify video asset exists; check model version.",
        ),
        (
            "226. Video asset dropdown empty",
            "Check valid status; verify session selected.",
        ),
        ...
        (
            "231. Cross-service filesystem mismatch",
            "Verify paths; use shared or absolute paths.",
        ),
    ...
    add_para(
        doc,
        "Increase max_execution_time in PHP config; verify video length; use interval > 1.",
    )

    add_heading_custom(doc, "237. PendingRequest::method does not exist", 3)
    add_para(
        doc,
        "Verified root cause: retry callback received PendingRequest instance (not Request object). Fixed in AiServiceClient.php by removing $request->method() call.",
    )
    ...
    add_para(
        doc, "VideoAssetPolicy registered; @can directives present; verify user role."
    )
    ...
    add_warning_box(
        doc,
        "B4 is partial/experimental; verify temporal evidence; do not auto-penalize.",
    )
    ...
    add_para(
        doc, "Commands labeled accordingly; PowerShell uses different syntax for pipes."
    )
    ...
    add_table_custom(
        ...
        [
            ...
            [
                ...
                "Partially implemented / partially verified",
                "Webcam/test verified; RTSP unverified",
            ],
            ...
            [
                "Temporal events B1-B3",
                "Partially implemented",
                "Rules exist; synthetic only",
            ],
    ...
    add_para(
        doc,
        "Implemented and runtime validated: recorded mode, detection, dashboard, audit, evidence, tests.",
    )
    add_para(
        doc,
        "Implemented but partially tested: live mode (RTSP unverified); temporal rules (synthetic only).",
    )
    add_para(doc, "Partially implemented: orientation, B4 event, training.")
    add_para(
        doc, "Experimental: geometric orientation, temporal model (if data justifies)."
    )
    add_para(
        doc,
        "Blocked: real participant evaluation; CI/dependabot; production deployment.",
    )
    ...
    add_warning_box(
        doc,
        "Not production-ready. Release acceptance criteria not met (CI/dependabot missing; user acceptance not fully signed off; full static analysis not executed; GPU benchmark unavailable). No version tag pushed. Remediation list documented in REMEDIATION_REPORT.md.",
    )
    ...
    add_table_custom(
        doc,
        ["Role", "Edit Video", "Del Video", "Manage Job", "Review", "Audit"],
        [
            ["system_admin", "Yes", "Yes", "Yes", "Yes", "Yes"],
    ...
    add_para(
        doc,
        "Key directories: ai-service/, dashboard/, docs/, research/, storage/, scripts/, .gitignored datasets/ and evidence/.",
    )

    add_heading_custom(doc, "279. Audit-Event Reference", 2)
    add_para(
        doc,
        "Video asset actions (upload/update/delete/blocked), job actions (create/approve/review/delete/retry), model actions.",
    )
    ...
    add_para(
        doc,
        "See DATA_PLAN.md, THREAT_MODEL.md, MODEL_BASELINE.md, BENCHMARKING.md, and repository documentation for full references.",
    )
